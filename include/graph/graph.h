...
class graph {
   ...
   public:
    ...
    //  returns the approximated and renormalized transition matrix used in arXiv:1609.02907 by Kipf and Welling
    //  K = \tilde{D}^{-1/2} * \tilde{A} * \tilde{D}^{-1/2}
    //  where \tilde{A} = A + I_{V} is the adjacency matrix of the arugmented graph with a self-loop of unit weight
    //  inserted to each vertex
    //  \tilde{D}_{ii} =  \sum_{k}\tilde{A}_{ik} is the (digonal) degree matrix
    //  \tilde{A} K_{ij} = \tilde{D}_{ii}^{-1/2} * \tilde{D}_{jj}^{-1/2} * ( A_{ij} + \delta_{ij} )
    //  currently just a workaround before appropriate math routines of sparse matrix addition/modification are
    //  implemented
