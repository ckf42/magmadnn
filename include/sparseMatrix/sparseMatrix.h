...
class spMatrix_DENSE : public sparseMatrix<T> {
   ...
   protected:
    Tensor<T>* _data;  //  the whole adjacency matix
...
//  since cusparse does not support row-major dense matrix while Tensor stores data in row-major order, internal _data after transposed and reshaped
//  i.e. _data->get_shape() == { _dim0, _dim1 }, adjMatrixTensorPtr->get() == _data->get()
//  hense whenever a object is created from a Tensor or whenever get_uncompressed_data() is called, extra time will be used to convect major order
